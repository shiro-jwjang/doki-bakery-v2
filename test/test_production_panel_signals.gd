...
func before_each() -> void:
	...
	# Create ProductionPanel instance from scene (not .new() - requires .tscn)
...
func test_panel_updates_on_baking_started() -> void:
	...
	assert_not_null(slot_ui.get_status_label(), "Slot should have a label")

	var label: Label = slot_ui.get_status_label()
...
func test_panel_updates_on_baking_finished() -> void:
	...
	assert_eq(slot_ui.get_progress_bar().value, 1.0, "Progress bar should be full")
...
func test_panel_updates_on_baking_progressed() -> void:
	...
	var slot_ui = panel.get_slot_ui(0)
	assert_not_null(slot_ui, "Slot UI should exist")
	assert_eq(slot_ui.get_progress_bar().value, 0.5, "Progress bar should show 50%")

	# Emit progress signal at 80%
	EventBus.production_progressed.emit(0, 0.8)
	await wait_physics_frames(2)

	assert_eq(slot_ui.get_progress_bar().value, 0.8, "Progress bar should update to 80%")
...
func test_panel_no_process_polling() -> void:
	...
	var slot_ui = panel.get_slot_ui(0)
	assert_not_null(slot_ui, "Slot UI should exist")
	assert_true(slot_ui.get_status_label().text.contains("완료"), "Slot should show completion without polling")
