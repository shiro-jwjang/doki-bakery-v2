...
const AvatarDataScript := preload("res://resources/avatar_data.gd")
...
func test_avatar_data_resource_exists() -> void:
	...
	assert_not_null(AvatarDataScript, "AvatarData script should exist")

	# AvatarData 인스턴스 생성 테스트
	var data: Resource = AvatarDataScript.new()
